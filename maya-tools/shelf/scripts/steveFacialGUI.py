...
def nameThings(prefix):
	...
	browMacroTemp = [prefix + 'cc_rEyeBrowOut_01', prefix + 'cc_rEyeBrowMid_01', prefix + 'cc_rEyeBrowInn_01', prefix + 'cc_rEyeBrowScrunch_01', prefix + 'cc_lEyeBrowScrunch_01', prefix + 'cc_lEyeBrowInn_01', prefix + 'cc_lEyeBrowMid_01', prefix + 'cc_lEyeBrowOut_01']
...
def go():
	...
	if cmds.objExists('papa_steve_rig_stable:cam_GUISTEVE_01') == True:
		prefix = 'papa_steve_rig_stable:'
		
	elif cmds.objExists('cam_GUISTEVE_01') == True: 
		prefix = ''
		
	else:
		confirm = cmds.confirmDialog(title='Error', message='Make sure Steve is in your scene.', button='Ok', defaultButton='Ok')
		return
	...
	newWindow = cmds.window('window', title = 'Steve Facial GUI', s=False, w=605)
	form = cmds.formLayout(w=605)
	steveEditor = cmds.modelEditor()
	steveAll = cmds.button(label = 'Key All', w=50, command = partial(keyAll, prefix))
	steveRowTop = cmds.rowLayout(parent = form, nc=2)
	steveClearAll = cmds.button(label = 'Clear All', w= 296, command = clearAll)
	steveResetAll = cmds.button(label = 'Reset All', w= 296, command = resetAll)
	cmds.setParent( '..' )
	steveColumn = cmds.columnLayout(w=605)
	...
	steveRow = cmds.rowLayout(parent = form, nc=3)
	...
	cmds.formLayout( form, edit=True, attachForm=[(steveEditor, 'top', 5), (steveEditor, 'left', 5), (steveAll, 'left', 5), (steveAll, 'right', 5), (steveRowTop, 'left', 5), (steveRowTop, 'right', 5), (steveRow, 'left', 5), (steveRow, 'bottom', 5), (steveRow, 'right', 5), (steveColumn, 'top', 5), (steveColumn, 'right', 5) ], attachControl=[(steveEditor, 'bottom', 5, steveAll), (steveColumn, 'bottom', 5, steveAll), (steveAll, 'bottom', 5, steveRowTop),(steveRowTop, 'bottom', 5, steveRow)], attachPosition=[(steveEditor, 'right', 0, 96), (steveColumn, 'left', 0, 96)], attachNone=(steveAll, 'top') )
	
	cmds.modelEditor( steveEditor, edit=True, camera= prefix + 'cam_GUISTEVE_01', wos=False, hud=False, gr=False, da = "smoothShaded", handles=False)
